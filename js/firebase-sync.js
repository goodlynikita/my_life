...
const FirebaseSync = (() => {
  ...
  async function pullIntoStore() {
    ...
    try {
      ...
      /* Firebase выбрасывает пустые массивы при сохранении, поэтому в базе
         может остаться «огрызок» без раздела training/plans. Считаем данные
         настоящими ТОЛЬКО если в них есть хотя бы один план. Иначе трактуем
         как пустую базу — и app.js восстановит данные из data.json. */
  ...
  async function pushNow() {
    const database = ensureInitialized();
    if (!database) return;
    /* НЕ перезаписываем базу стором без планов — иначе снова получим
       «огрызок» {meta} и потеряем данные. */
    const d = Store.get();
    const hasPlans = d && d.training && Array.isArray(d.training.plans) && d.training.plans.length > 0;
    if (!hasPlans) {
      console.warn('FirebaseSync.pushNow: пропущено — в сторе нет планов, база не тронута');
      return;
    }
