...
const FIN_LABEL_COLORS = {
  ...
  'blue':   { hex: '#2E7FD4', name: 'Синий (другой источник)' },
...
window.Screens.finance = function(mount) {
  ...
  function renderBalance() {
    ...
    content.innerHTML = `
      ...
      ${toSet > 0 ? `
      <div class="bal-reserve-card">
        <div class="bal-reserve-icon">💰</div>
        <div>
          <div class="bal-reserve-title">Нужно ещё отложить</div>
          <div class="bal-reserve-amt">${finFmtFull(toSet)}</div>
          <div class="bal-reserve-sub">чтобы покрыть все минимальные расходы</div>
        </div>
      </div>` : `
      <div class="bal-reserve-card bal-reserve-ok">
        <div class="bal-reserve-icon">✅</div>
        <div>
          <div class="bal-reserve-title">Минимум покрыт</div>
          <div class="bal-reserve-amt" style="color:#16A34A;">${finFmtFull(cushion)}</div>
          <div class="bal-reserve-sub">свободных денег сверх минимума</div>
        </div>
      </div>`}
