...
window.Screens.habits = function(mount) {
  ...
  function wheelDrawSVG(scores, size, interactive) {
    ...
    const labels = WHEEL_SPHERES_DEFAULT.map((name,i) => {
      ...
      return `<text x="${x}" y="${y}" text-anchor="${anchor}" dominant-baseline="middle" font-size="9" fill="#6B7280" font-family="Montserrat,sans-serif">${shortName}</text>`;
    }).join('');
    ...
    const dots = scores.map((s,i) => {
      ...
      const x = cx + r*Math.cos(a);
      const y = cy + r*Math.sin(a);
      return `<circle cx="${x}" cy="${y}" r="4" fill="#9333EA" stroke="#fff" stroke-width="1.5" ${interactive?`class="wheel-dot" data-i="${i}"`:''}/>`;
    }).join('');

    return `<svg width="${size}" height="${size}" viewBox="0 0 ${size} ${size}" xmlns="http://www.w3.org/2000/svg">
      ${gridLines}${axes}
      <polygon points="${dataPoints}" fill="#9333EA22" stroke="#9333EA" stroke-width="2" stroke-linejoin="round"/>
      ${dots}${labels}
      <text x="${cx}" y="${cy-6}" text-anchor="middle" font-size="18" font-weight="800" fill="#111" font-family="Montserrat,sans-serif">${avg}</text>
      <text x="${cx}" y="${cy+10}" text-anchor="middle" font-size="9" fill="#9CA3AF" font-family="Montserrat,sans-serif">средний балл</text>
    </svg>`;
  }
